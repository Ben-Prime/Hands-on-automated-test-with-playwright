...
- Name: chapter_5\broken_images_test.spec.ts >> Broken Images Validation >> should not find any broken images on the page
- Location: tests\chapter_5\broken_images_test.spec.ts:5:9

# Error details

```
Error: Found 3 broken images:
[
  {
    "src": "https://with-bugs.practicesoftwaretesting.com/broken.png",
    "status": 404
  },
  {
    "src": "https://with-bugs.practicesoftwaretesting.com/broken.png",
  ...
  {
    "src": "https://with-bugs.practicesoftwaretesting.com/assets/img/products/pliers05.jpeg",
...
```ts
  ...
  2  | 
  3  | type ImageValidatorFixture = {
  4  |   imageValidator: () => Promise<void>;
  5  | };
  6  | 
  7  | export const test = base.extend<ImageValidatorFixture>({
  8  |   imageValidator: async ({ page }, use) => {
  9  |     const validateImages = async () => {
  10 |       const allImages = await page.locator('img').all();
  11 |       const brokenImages = [];
  12 | 
  13 |       for (const img of allImages) {
  14 |         const src = await img.getAttribute('src');
  15 | 
  16 |         if (!src || src.trim() === '') {
  17 |           brokenImages.push({
  18 |             src: 'Missing src attribute',
  19 |             status: 'N/A',
  20 |           });
  21 |           continue;
  22 |         }
  23 | 
  24 |         const absoluteSrc = new URL(src, page.url()).href;
  25 | 
  26 |         try {
  27 |           const response = await page.request.get(absoluteSrc);
  28 | 
  29 |           if (!response.ok()) {
  30 |             brokenImages.push({
  31 |               src: absoluteSrc,
  32 |               status: response.status(),
  33 |             });
  34 |           } else {
  35 |             const isCorrupted = await img.evaluate(
  36 |               (image: HTMLImageElement) => image.naturalWidth === 0
  37 |             );
  38 | 
  39 |             if (isCorrupted) {
  40 |               brokenImages.push({
  41 |                 src: absoluteSrc,
  42 |                 status: 'OK but corrupted (naturalWidth is 0)',
  43 |               });
  44 |             }
  45 |           }
  46 |         } catch (error) {
  ...
  60 |         `Found ${brokenImages.length} broken images:\n` +
  61 |         JSON.stringify(brokenImages, null, 2);
  62 | 
> 63 |       expect(brokenImages, errorMessage).toEqual([]);
     |                                          ^ Error: Found 3 broken images:
  64 |     };
  65 | 
  66 |     await use(validateImages);
  67 |   },
  68 | });
```
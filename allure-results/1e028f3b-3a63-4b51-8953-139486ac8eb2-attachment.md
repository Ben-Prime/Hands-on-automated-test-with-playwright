...
- Name: chapter_15\implementing_the_fixture_using_config_test.spec.ts >> access cart after login
- Location: tests\chapter_15\implementing_the_fixture_using_config_test.spec.ts:3:5
...
```ts
  1 | import { test, expect } from '../../fixtures/login-fixture-using-config.fixture';
  2 | 
  3 | test('access cart after login', async ({ loggedInPage }) => {    
  4 | //await loggedInPage.goto('https://www.saucedemo.com/cart.html');
  5 | // Make sure Checkout button is visible
> 6 | await expect(loggedInPage.getByRole('button', { name: 'Checkout' })).toBeVisible();
    |                                                                      ^ Error: expect(locator).toBeVisible() failed
  7 | });
```
...
test('should upload multiple files successfully', async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: 'sample1.jpg' }).first()).toBeVisible();
    await expect(page.getByRole('link', { name: 'sample2.jpg' }).first()).toBeVisible();
    await expect(page.getByRole('link', { name: 'sample3.jpg' }).first()).toBeVisible();
});
